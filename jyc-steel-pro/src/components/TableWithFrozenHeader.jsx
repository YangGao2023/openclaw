...
export default function TableWithFrozenHeader({ columns, data, onRowClick, selectedId, emptyText = "暂无数据", headerColor = "bg-blue-600" }) {
  return (
        ...
        <Table style={{ borderCollapse: 'collapse', width: '100%', tableLayout: 'fixed' }}>
          <TableBody>
            {data.length === 0 ? (
              ...
              data.map((row, idx) => {
                const isHighlighted = selectedId === row.id;
                return (
                  <TableRow 
                    key={row.id || idx} 
